...
shinyServer(function(input, output, clientData, session) {
  ...
  updateSelectInput(session, "paxes", choices = varRange, selected = varRange[c(1,2)])
  ...
  resetPlotOptions <- observeEvent(input$resetOptions, {
    ...
    updateSelectInput(session, "paxes", selected = varRange[c(1,2)])
  ...
  # Parallel Coordinates Tab ----------------------------------------------------------------
   output$parallelAxisPlot <- renderPlot({
       vars <- paxesList()
       data <- colorData()
    validate(need(length(vars)>=2, "Please select two or more display variables."))
     ...
     print("Rendering Plot.")
       #pairs(data[vars],lower.panel = panel.smooth,upper.panel=NULL, col=data$color, pch = as.numeric(input$pointStyle), cex = as.numeric(input$pointSize))
       parcoord(data[vars], col = 1 + (0:149)%/%50,var.label=TRUE)
     # }
     print("Plot Rendered.")
   })
  ...
  varsList <- reactive({
    ...
    print("Getting Variable List.")
    idx = 0
    for(choice in 1:length(input$paxes)) {
      mm <- match(input$paxes[choice],varNames)
      if(mm > 0) { idx <- c(idx,mm) }
    }
    print(idx)
    idx
  ...
  output$exportData <- downloadHandler(
    ...
    content = function(file) { write.csv(filterData(), file, row.names=FALSE) }
  )
  
  output$exportRanges <- downloadHandler(
    filename = function() { paste('ranges-', Sys.Date(), '.csv', sep='') },
    content = function(file) { write.csv(rangesData(), file, row.names=FALSE) }
  )
